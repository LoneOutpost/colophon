...
adapter, writes it next to the book as `cover-<book id>.<ext>` (keyed on the book
id so books sharing a folder never collide), and sets `cover_path`.
...
async def ensure_cached_cover(
    ...
    if cover is None:
        return None
    # Key the cache file on the book id, not the folder: clustered books share one
    # source folder, so a folder-keyed "cover.<ext>" would collide and every book in
    # the folder would serve whichever cover was written last.
    path = dest_dir / f"cover-{book.id}{ext_for_mime(cover.mime)}"
    try:
