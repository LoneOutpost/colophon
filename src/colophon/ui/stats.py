...
@contextmanager
def _section(title: str) -> Iterator[None]:
...
def render_stats(controller: AppController) -> None:
    ...
    with ui.column().classes("w-full items-center q-pa-md"):
        with ui.column().classes("w-full gap-4").style("max-width: 900px"):
            if not books:
                with ui.card().classes("w-full q-pa-md"):
            ...
            with ui.row().classes("w-full q-gutter-md items-stretch"):
                for kind, label in _TOP_KINDS:
                    ...
                    with ui.column().classes("col"), _section(label):
                        if not entries:
