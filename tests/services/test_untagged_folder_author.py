...
self-heal on a REFRESH after a bad sidecar is removed. Real TE_Audiobooks_S shape, no network."""
...
def _build(root: Path, tree: dict[str, list[str]], *, sidecar: dict | None = None) -> None:
    for author, files in tree.items():
        ...
        if sidecar is not None:
            (d / "metadata.json").write_text(json.dumps(sidecar), encoding="utf-8")
...
def test_refresh_heals_stranded_author_after_bad_sidecar_removed(tmp_path):
    ...
    _build(root, tree, sidecar={"title": "Placeholder Collection", "authors": ["TE_Audiobooks_S"]})

    conn = connect(tmp_path / "db.sqlite")
    migrate(conn)
    repo = BookUnitRepo(conn)

    # Phase 1: the bad sidecar leaks the bucket as author.
    plan = plan_scan_graph(repo, root, template="$Author - $Title", directory_scheme="")
    commit_scan(repo, plan, reconcile=True)
    assert _authors_by_folder(repo)["stella Rimington"] == {"TE_Audiobooks_S"}

    # Operator removes the erroneous sidecar.
